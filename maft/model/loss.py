    ...
    def forward(self, pred_labels, pred_masks, pred_bboxes, insts):
        ...
        for pred_label, pred_mask, pred_bbox, inst in zip(pred_labels, pred_masks, pred_bboxes, insts):
            ...
            cost_bbox = torch.cdist(
                pred_bbox, inst.gt_bboxes[:, :3], p=1
            )

            C = (self.cost_weight[0] * cost_class + self.cost_weight[1] * cost_mask + self.cost_weight[2] * cost_dice + self.cost_weight[3] * cost_bbox)
            C = C.cpu()
...
class Criterion(nn.Module):
    ...
    def get_layer_loss(self, layer, aux_outputs, insts):
        ...
        loss = (
            ...
            self.loss_weight[2] * mask_dice_loss + self.loss_weight[3] * bbox_loss + self.loss_weight[4] * score_loss)
